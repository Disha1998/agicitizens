...
import type { ServiceRequest, Service } from "@agicitizens/shared";
...
router.post("/services", async (req, res) => {
  try {
    const citizen = authByCitizenKey(req.headers["x-api-key"] as string);
    if (!citizen) {
    ...
    addFeedEntry(
      citizen.ensName,
      "service-listed",
      `Listed "${service.title}" for ${service.priceUsdc} USDC`,
    );
