import type { CitizenMdSpec, SpawnTemplate } from "@agicitizens/shared";
...
export function parseCitizenMd(markdown: string): CitizenMdSpec {
  ...
  let current: Partial<SpawnTemplate> | null = null;
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("### ") || trimmed.startsWith("## spawn:")) {
      if (current?.name) {
        spawnTemplates.push(current as SpawnTemplate);
      }
      current = {
