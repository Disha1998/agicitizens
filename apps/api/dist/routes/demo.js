 ...
 */
function autoRateTask(taskId) {
    // Run async so we don't block the demo response
    rateTaskWithAI(taskId).catch((err) => console.warn("[demo/rate] AI rating failed:", err.message));
}
const RATING_SYSTEM_PROMPT = 'You are an AI agent evaluating another agent\'s task completion. Respond with JSON only: { "rating": number (1-5), "review": string }. Be fair but encouraging. Consider speed, accuracy, and cost-effectiveness.';
async function getAIRating(taskDescription) {
...
async function rateTaskWithAI(taskId) {
    ...
    if (target) {
        ...
        const ratedTasks = [...tasks.values()].filter((t) => t.toEns === target.ensName && t.rating !== null);
        ...
        target.reputationScore = Math.min(100, Math.round(50 + target.avgRating * 6 + Math.min(target.tasksCompleted, 50)));
        addFeedEntry(task.fromEns, "rate", `Rated ${target.ensName} ${rating}/5 — "${review}"`);
    }
    saveState();
 ...
 */
async function ensureParentFunded(amount) {
    ...
    try {
        ...
        if (parent && agent.wallet) {
            ...
            try {
                ...
                addFeedEntry(parent.ensName, "hire", `Paid ${agent.ensName} for research on ${token} — 0.50 USDC`, txHash);
                // Update stats
...
router.get("/demo/agents", async (_req, res) => {
    ...
    const svcList = [...services.values()]
        ...
        if (aOwner?.category === "DeFi Execution")
            return -1;
        if (bOwner?.category === "DeFi Execution")
            return 1;
        return 0;
    })
        .map((s) => ({
