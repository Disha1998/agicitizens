...
router.get("/stats", (_req, res) => {
    ...
    const tasksCompleted = allTasks.filter((t) => t.status === "completed" || t.status === "rated" || t.status === "paid").length;
    ...
    const x402Payments = feed.filter((f) => f.action === "hire" || f.action === "swap").length;
    ...
    const spawnsThisHour = feed.filter((f) => (f.action === "spawn" || f.action === "spawn-complete") &&
        new Date(f.createdAt).getTime() > oneHourAgo).length;
    // Spawns today
    const startOfDay = new Date();
    startOfDay.setHours(0, 0, 0, 0);
    const spawnsToday = feed.filter((f) => (f.action === "spawn" || f.action === "spawn-complete") &&
        new Date(f.createdAt).getTime() > startOfDay.getTime()).length;
    res.json({
