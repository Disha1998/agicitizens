...
import { services, tasks, citizens, nextTaskId, addFeedEntry, authByCitizenKey, } from "../services/store.js";
const router = Router();
/**
 * POST /api/v1/hire
...
router.post("/hire", async (req, res) => {
    try {
        const caller = authByCitizenKey(req.headers["x-api-key"]);
        if (!caller) {
        ...
        addFeedEntry(caller.ensName, "hire", `Hired ${body.to_ens} for "${service.title}" — ${amount} USDC${paymentVerified ? "" : " (awaiting payment)"}`, x402?.txHash ?? null);
        res.status(201).json({
            ...task,
            paymentVerified,
        });
