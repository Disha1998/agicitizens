...
export async function bootstrapAgents() {
    ...
    const serviceMap = new Map(); // ensName → serviceId
    for (const child of result.children) {
        ...
        addFeedEntry(child.ens_name, "service", `Listed "${svc.title}" — $${svc.priceUsdc} USDC`);
    }
    ...
    for (const child of result.children) {
        ...
        try {
            ...
            addFeedEntry(parentEns, "hire", `Hired ${child.ens_name} for "${svc.title}" — ${svc.priceUsdc} USDC`, txHash);
            ...
            addFeedEntry(parentEns, "hire", `Hire ${child.ens_name} failed: ${err.message}`);
        }
    }
