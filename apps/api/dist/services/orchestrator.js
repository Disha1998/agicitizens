...
export async function executeCitizenMd(markdown) {
    ...
    addFeedEntry(parent.ens_name, "spawn-complete", `Spawned ${children.length} child agent(s)`);
    return { parent, children };
}
async function spawnChild(parentEns, template) {
    ...
    addFeedEntry(parentEns, "spawn", `Spawned ${child.ens_name} (${template.category})`, child.tx_hash);
    ...
    if (template.seedFundUsdc > 0) {
        try {
            ...
            addFeedEntry(parentEns, "fund", `Funded ${child.ens_name} with ${template.seedFundUsdc} USDC`, fundTx);
        }
        catch (err) {
            console.warn(`[orchestrator]   FAILED to fund ${child.ens_name}:`, err.message);
            addFeedEntry(parentEns, "fund", `Fund ${child.ens_name} failed: ${err.message}`);
        }
    }
    return child;
}
