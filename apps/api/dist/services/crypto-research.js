...
export async function runResearch(agentEns, query) {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
            ...
            system: "You are a crypto research analyst. Respond with JSON: { \"analysis\": string, \"sentiment\": \"bullish\"|\"bearish\"|\"neutral\", \"confidence\": number (0-100), \"sources\": string[] }. Be concise and data-driven.",
        }),
    });
    if (!response.ok) {
    ...
    try {
        ...
        addFeedEntry(agentEns, "research", `Analyzed ${query.token} — ${result.sentiment}`);
        return result;
    }
}
